package com.dds.loftcoins.ux.widgets;
...
public class CircleIndicator extends RecyclerView.ItemDecoration {
    ...
    public CircleIndicator(@NonNull Context context) {
        ...
        radius = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 4, dm);
    ...
    public void onDrawOver(@NonNull Canvas c, @NonNull RecyclerView parent, @NonNull RecyclerView.State state) {
        ...
        if (adapter != null) {
            ...
            float posY = parent.getHeight() - 2 * radius;
            ...
            for (int i = 0; i < adapter.getItemCount(); ++i) {
                drawIndicator(c, posX + 4 * radius * i, posY, currentIndicator == i);
            }
        }
    }

    private void drawIndicator(@NonNull Canvas c, float x, float y, boolean active) {
        c.drawCircle(x, y, radius, active ? activePaint : inactivePaint);
    }
}
